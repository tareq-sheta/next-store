...
// import { login, register } from "@/handlers/users"; // once fixed per #3
...
export default function LoginPage() {
  ...
  const onSubmit: SubmitHandler<FormInputs> = async (data) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(
          result.error ?? `${isLogin ? "Login" : "Registration"} failed.`,
        );
        return;
      }
        ...
        <div className="max-w-md w-full mx-auto relative">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            {/* <div className={registerFieldClass}>
              ...
              <select {...reg("role")} className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm bg-white outline-none focus:ring-2 focus:ring-gray-400">
              ...
              <div className="grid grid-cols-2 gap-3 h-10.5">
                <button
                  ...
                  className={`py-2 rounded-lg text-xs font-bold border transition-all duration-300 h-full ${
                    watch("role") === "customer"
                      ? // Active state: Matching the Login button (black/white)
                        "bg-gray-950 border-gray-950 text-white shadow-md"
                      : // Inactive state: Subtle gray border and text, white bg
