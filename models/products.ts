...
//     category: {
//   type: String,
//   enum: ["phones", "smartwatch", "headphones", "cameras", "computers", "gaming", "others"],
...
export const productsModel: Model<IProduct> =
  mongoose.models.Product || mongoose.model<IProduct>("Product", productsSchema);
...
class Products {
  ...
  async showOne(query: string): Promise<ProductDoc | null> {
    try {
      ...
      throw new CustomError(`DB error looking up product: ${query}`, 500, "products.showOne");
    }
  }
  ...
  async create(obj: Partial<IProduct>): Promise<ProductDoc> {
    try {
      ...
      if (err && typeof err === "object" && "code" in err && err.code === 11000) {
        throw new CustomError("Product name already exists", 400, "products.create");
      }
      throw new CustomError("Failed to create product", 500, "products.create");
    }
  }
  ...
  async update(id: string, data: Partial<IProduct> & { _id?: unknown }): Promise<ProductDoc | null> {
    try {
      const updateData = omitId(data);
      return await productsModel
